...
jq '.transports.docker["ghcr.io/renner0e"] = [{"type":"sigstoreSigned","keyPaths":["/etc/pki/containers/renner.pub","/etc/pki/containers/renner.pub"],"signedIdentity":{"type":"matchRepository"}}]' /etc/containers/policy.json > /etc/containers/policy.json
